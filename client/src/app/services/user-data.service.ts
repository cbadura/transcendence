...
export class UserDataService {
  ...
  private serverAddress: string = `http://${import.meta.env['NG_APP_HOST_NAME']}:3000`;
  ...
  console.log('IN NEWEST USER')
    const url = `http://${import.meta.env['NG_APP_HOST_NAME']}:3000/auth/profile`;
  ...
  removeRelation(relationID: number): Observable<any> {
    const url = `http://${import.meta.env['NG_APP_HOST_NAME']}:3000/relationship/${relationID}`;
...
//   }
  ...
  CreateSocketConnections() {
    ...
    const gameUrl = `http://${import.meta.env['NG_APP_HOST_NAME']}:3000/game`;
    if (!this.gameSocket) {
    ...
    if (!this.chatSocket) {
      this.chatSocket = new Socket({
        url: `http://${import.meta.env['NG_APP_HOST_NAME']}:3000/chat`,
        options: {
          withCredentials: true,
          forceNew: true
      } });
    ...
    if (!this.userSocket) {
      this.userSocket = new Socket({
        url: `http://${import.meta.env['NG_APP_HOST_NAME']}:3000/`,
        options: {
          withCredentials: true,
          forceNew: true
        } });
