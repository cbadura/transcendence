...
export class RelationshipService {
  ...
  private async getRelationship(id: number): Promise<Relationship> {
    const relation: Relationship = await this.relationshipRepository.findOne({where: { id }});
    if (!relation)
      throw new NotFoundException(`Relationship with ID ${id} not found.`);
    return relation;
  }

  async createOrUpdateRelationship(reqUserId: number, dto: CreateRelationshipDto): Promise<Relationship> {
    //check if both IDs are valid REFACTOR THIS CODE TO HAVE A GENERIC MODULE THAT HANDLES VALIDATION OF EXISTING RECORDS
    if (reqUserId !== dto.user_id)
  ...
  async deleteRelationship(reqUserId: number, relationship_id: number) {
    const remove = await this.getRelationship(relationship_id);
    if (reqUserId !== remove.primary_user_id)
      throw new UnauthorizedException();
    ...
    // }
    await this.relationshipRepository.delete(relationship_id);
  ...
  async generateDebugRelationships(id: number,relationship_status: 'friend' | 'blocked'){
    for (let i = 0; i < 10; i++) {
      ...
      await this.createOrUpdateRelationship(id, dto);
    }
  }
